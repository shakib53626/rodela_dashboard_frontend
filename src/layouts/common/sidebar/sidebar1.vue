  ...
  <div class="sidebar-wrapper" :class="isSidebarOpen ? '' : 'close_icon'">
    <div>
      <div class="logo-wrapper logo-wrapper-center">
        <router-link to="/" href="javascript:void(0)" data-bs-original-title="" title="" >
      ...
      <nav class="sidebar-main">
        ...
        <div id="sidebar-menu">
          <ul class="sidebar-links" id="simple-bar">
            ...
            <li v-for="(item, index) in sidebarLinks" :key="index" :class="item.application ? item.mainClasses : 'sidebar-list'" >
              <div v-if="item.application">
                <h6 class="lan-1">{{ item.name }}</h6>
                <p class="lan-2">{{ item.subHeading }}</p>
              </div>
              <a v-else-if="item.children" :class="[ item.class ? item.class : 'sidebar-link sidebar-title', item.active ? 'active' : '', ]" href="javascript:void(0)" @click.prevent="toggleSubmenu(item.name)" >
              ...
              <router-link v-else :class="[ item.class ? item.class : 'sidebar-link sidebar-title link-nav', item.active ? 'active' : '', ]" href="javascript:void(0)" :to="item.path" @click="currentActiveMenu = item.name" >
                <vueFeather :type="item.icon" v-if="item.icon"></vueFeather>
                <span>{{ item.name }}</span>
              </router-link>

              <ul class="sidebar-submenu" v-if="item.children" :class="item.active ? '' : 'hideSubmenu'" >
                <li v-for="(link, index) in item.children" :key="'a' + index">
                  <router-link :to="link.path" :class="[{ active: item.active }]" >{{ link.name }}</router-link >
                </li>
              </ul>
            </li>
